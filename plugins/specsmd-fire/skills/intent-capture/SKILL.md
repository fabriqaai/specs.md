---
name: intent-capture
description: Use when the user wants to start something new in FIRE and no active intent exists. Captures the what and why through guided conversation and writes an intent brief.
license: MIT
metadata:
  version: "1.0.0"
  flow: fire
...
<flow>
  ...
  <step n="4" title="Generate Intent Brief">
    ...
    <action>Generate intent brief using the bundled template `references/brief.md.hbs` in this skill's directory</action>
  ...
  <step n="6" title="Transition">
    ...
    <check if="response == y">
      <invoke_skill>the `work-item-decompose` skill</invoke_skill>
    </check>
  </step>
</flow>
...
<output_artifacts>
  ...
  |----------|----------|----------|
  | Intent Brief | `.specs-fire/intents/{id}/brief.md` | `references/brief.md.hbs` |
</output_artifacts>
