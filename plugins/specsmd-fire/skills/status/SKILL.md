---
name: status
description: Use when the user asks for FIRE project status, or to check or validate FIRE state. Reports intents, work items, and runs, then validates state.yaml against the file system and offers fixes.
license: MIT
metadata:
  version: "2.0.0"
  flow: fire
...
# FIRE Status and Integrity Check
...
<flow>
  <step n="1" title="Read State">
    ...
    <check if="state.yaml not found">
      <output>No FIRE project found. Invoke the `fire-init` skill to initialize.</output>
      <exit/>
    </check>
  </step>
  ...
  <step n="15" title="Final Summary">
    <output>
      ...
      **Issues Remaining**: {remaining_count}
      ...
      {else}
      ⚠️ Some issues remain unresolved. Invoke the `status` skill again to review.
      {/if}
