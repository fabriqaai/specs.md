---
name: run-plan
description: Use when pending FIRE work items exist and the user wants to start execution, or right after decomposition. Discovers work on disk, reconciles state, and lets the user choose run scope.
license: MIT
metadata:
  version: "1.0.0"
  flow: fire
...
<schema>
Artifact paths, run-scope definitions, and execution modes come from the memory-bank
schema in `references/memory-bank.yaml` in this skill's directory.
</schema>
...
<flow>
  ...
  <step n="7" title="Confirm Run">
    ...
    <check if="response == y">
      <invoke_skill args="work_items_for_run, run_scope">the `run-execute` skill</invoke_skill>
    </check>
  </step>
</flow>
