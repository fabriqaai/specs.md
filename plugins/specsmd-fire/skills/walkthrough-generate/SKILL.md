...
description: Use after a FIRE run completes, or when the user asks for an implementation walkthrough of a run. Documents changes, decisions, deviations, and verification steps for human review.
license: MIT
metadata:
  version: "1.0.0"
  flow: fire
...
<flow>
  <step n="1" title="Gather Implementation Data">
    ...
    <action>Read the work item file from .specs-fire/intents/{intent}/work-items/{work-item-id}.md (work items are files, not directories)</action>
  ...
  <step n="6" title="Generate Walkthrough">
    <action>Generate walkthrough using the bundled template `references/walkthrough.md.hbs` in this skill's directory</action>
