...
description: Use when a FIRE work item is high complexity or in Validate mode and needs design review before implementation. Produces a design doc for Checkpoint 1 approval.
license: MIT
metadata:
  version: "1.0.0"
  flow: fire
...
<flow>
  ...
  <step n="9" title="Handle Response">
    <check if="response == y">
      <action>Generate design doc using the bundled template `references/design.md.hbs` in this skill's directory</action>
      ...
      <output>
        ...
        Continue to the `run-plan` skill for Checkpoint 2 (implementation plan)? [Y/n]
      </output>
    </check>
...
<output_artifacts>
  ...
  |----------|----------|----------|
  | Design Doc | `.specs-fire/intents/{intent-id}/work-items/{id}-design.md` | `references/design.md.hbs` |
</output_artifacts>
