...
description: Use when a FIRE intent exists without work items, or the user wants to plan execution of an intent. Breaks the intent into executable work items with complexity, execution mode, and validated dependencies.
license: MIT
metadata:
  version: "1.0.0"
  flow: fire
...
<schema>
Artifact paths, complexity→mode mapping, and the autonomy-bias table come from the
memory-bank schema in `references/memory-bank.yaml` in this skill's directory.
</schema>
...
<flow>
  ...
  <step n="7" title="Save Work Items">
    <check if="approved">
      ...
      <action>For each work item, generate using the bundled template `references/work-item.md.hbs` in this skill's directory</action>
...
<output_artifacts>
  ...
  |----------|----------|----------|
  | Work Item | `.specs-fire/intents/{intent-id}/work-items/{id}.md` | `references/work-item.md.hbs` |
</output_artifacts>
