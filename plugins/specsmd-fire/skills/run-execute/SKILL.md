---
name: run-execute
description: Use when a FIRE run is ready to execute or an interrupted run needs resuming. Executes work items in autopilot, confirm, or validate mode and produces the run artifacts.
license: MIT
metadata:
  version: "1.0.0"
  flow: fire
...
<prerequisites>
  The four `.cjs` scripts this skill uses are bundled in this skill's `scripts/`
  directory. Resolve their absolute path from where this skill file lives — do
  NOT expect them to exist in the user's project.

  They require Node and the `yaml` package.

  <step n="1" title="Locate Scripts">
    <action>Determine SCRIPTS_DIR = the absolute path of the `scripts/` directory next to this skill file</action>
    <action>Invoke every script as: node {SCRIPTS_DIR}/{script}.cjs ...</action>
  </step>

  <step n="2" title="Check the yaml Package">
    <action>Check whether `yaml` is resolvable from SCRIPTS_DIR: node -e "require.resolve('yaml')" (run with cwd = SCRIPTS_DIR)</action>
    <check if="resolution fails">
      <critical>NEVER install dependencies into the user's project for this skill.</critical>
      <output>Installing the `yaml` package into the skill's own directory (not your project).</output>
      <action>Run: npm install --prefix {SCRIPTS_DIR} yaml</action>
      <action>If SCRIPTS_DIR is not writable, tell the user and stop — do not fall back to installing in their project</action>
    </check>
  </step>

  | Package | Purpose | Install location |
  |---------|---------|------------------|
  | `yaml` | Parse/stringify state.yaml | This skill's `scripts/` directory, via `npm install --prefix {SCRIPTS_DIR} yaml` |
...
<flow>
  <step n="1" title="Initialize Run">
    ...
    <code>
      # For single item:
      node {SCRIPTS_DIR}/init-run.cjs {rootPath} {workItemId} {intentId} {mode}

      # For batch/wide (multiple items):
      node {SCRIPTS_DIR}/init-run.cjs {rootPath} --batch '[
  ...
  <step n="3b" title="Confirm Mode" if="mode == confirm">
    ...
    <action>Save plan IMMEDIATELY using the bundled template `references/plan.md.hbs` in this skill's directory</action>
    ...
    <code>node {SCRIPTS_DIR}/update-checkpoint.cjs {rootPath} {runId} awaiting_approval --checkpoint=plan</code>
    ...
    <code>node {SCRIPTS_DIR}/update-checkpoint.cjs {rootPath} {runId} approved --checkpoint=plan</code>
    <goto step="5"/>
  </step>
  ...
  <step n="4" title="Generate Plan (Autopilot Only)" if="mode == autopilot">
    ...
    <batch_handling critical="true">
      ...
      <check if="plan.md does not exist OR single run">
        <action>Generate implementation plan</action>
        <action>Save plan using the bundled template `references/plan.md.hbs` in this skill's directory</action>
        <action>Write to: .specs-fire/runs/{run-id}/plan.md</action>
      </check>
    </batch_handling>
  ...
  <step n="5" title="Execute Implementation">
    ...
    <code>node {SCRIPTS_DIR}/update-phase.cjs {rootPath} {runId} execute</code>
  ...
  <step n="6" title="Run Tests">
    ...
    <code>node {SCRIPTS_DIR}/update-phase.cjs {rootPath} {runId} test</code>
    ...
    <batch_handling critical="true">
      ...
      <check if="test-report.md does not exist OR single run">
        <action>Generate test report using the bundled template `references/test-report.md.hbs` in this skill's directory</action>
  ...
  <step n="6b" title="Code Review">
    ...
    <code>node {SCRIPTS_DIR}/update-phase.cjs {rootPath} {runId} review</code>
    ...
    <invoke_skill>the `code-review` skill</invoke_skill>
  ...
  <step n="7" title="Complete Current Work Item">
    ...
    <substep n="7b" title="Route Based on Batch Status">
      <check if="scope in [batch, wide] AND pending_count > 0">
        ...
        <code>
          node {SCRIPTS_DIR}/complete-run.cjs {rootPath} {runId} --complete-item
        </code>
      ...
      <check if="scope == single OR pending_count == 0">
        ...
        <code>
          node {SCRIPTS_DIR}/complete-run.cjs {rootPath} {runId} --complete-run \
  ...
  <step n="8" title="Generate Walkthrough">
    <invoke_skill>the `walkthrough-generate` skill</invoke_skill>
  </step>
...
<scripts>
  All four scripts are bundled in this skill's `scripts/` directory. Resolve the
  absolute path from where this skill file lives (SCRIPTS_DIR) before invoking them.
  ...
  | `scripts/update-phase.cjs` | Update current work item's phase | `node {SCRIPTS_DIR}/update-phase.cjs {rootPath} {runId} {phase}` |
  | `scripts/complete-run.cjs` | Finalize run and update state | `--complete-item` or `--complete-run` |

  <script name="init-run.cjs">
    ```bash
    # Single work item
    node {SCRIPTS_DIR}/init-run.cjs /project work-item-id intent-id autopilot

    # Batch/wide (multiple items)
    node {SCRIPTS_DIR}/init-run.cjs /project --batch '[
  ...
  <script name="complete-run.cjs">
    ...
    node {SCRIPTS_DIR}/complete-run.cjs /project run-fabriqa-2026-001 --complete-item

    # Complete entire run (single runs or final item in batch)
    node {SCRIPTS_DIR}/complete-run.cjs /project run-fabriqa-2026-001 --complete-run \
