...
description: Use when the user asks to write or update the requirements document for a feature spec, or when a new feature idea starts a spec. Produces `specs/{feature}/requirements.md` with EARS-format acceptance criteria.
license: MIT
metadata:
  version: "1.0.0"
  flow: simple
...
|----------|---------|------------|
| `references/requirements-template.md` (this skill's directory) | Document structure for the requirements file | Warn and continue using the structure described below |
| `specs/{feature}/requirements.md` | Existing requirements, when updating rather than creating | Warn and continue — treat as a new spec |
...
See `references/requirements-template.md` in this skill's directory for full template structure.
