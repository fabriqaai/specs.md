---
name: spec-execute
description: Use when the user asks to implement, execute, or continue a task from an approved feature spec, or asks which task comes next. Executes one task at a time against the spec and updates its checkbox in `specs/{feature}/tasks.md`.
license: MIT
metadata:
  version: "1.0.0"
  flow: simple
...
Read all three before executing anything:

| Artifact | Purpose | If missing |
|----------|---------|------------|
| `specs/{feature}/requirements.md` | What to build, and the acceptance criteria to verify against | Stop and tell the user the spec is incomplete |
| `specs/{feature}/design.md` | How to build it — components, interfaces, data models | Stop and tell the user the spec is incomplete |
| `specs/{feature}/tasks.md` | Current progress and the task to execute | Stop and tell the user the spec is incomplete |
...
3. Suggest resolution:
   ...
   - Design gap → invoke the `spec-design` skill
   - Requirement unclear → invoke the `spec-requirements` skill
