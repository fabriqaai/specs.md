---
name: simple-spec
description: Use when the user asks to create, update, resume, or implement a feature specification — phrasings like "create a spec for X", "write requirements for the login system", "continue the user-auth spec", "execute task 3.2", or "what's the next task". Guides lightweight spec-driven development from requirements through design, tasks, and execution, storing artifacts under `specs/{feature}/` with explicit user approval between phases.
license: MIT
metadata:
  version: "1.0.0"
  flow: simple
...
Spec-driven development for a single feature. Transform a feature idea into structured specifications with requirements, design, and implementation tasks, then execute those tasks one at a time.

The workflow runs in three phases plus execution:

1. **Requirements** — Define what to build with EARS-format acceptance criteria (`spec-requirements` skill)
2. **Design** — Create technical design with architecture and data models (`spec-design` skill)
3. **Tasks** — Generate implementation checklist with coding tasks (`spec-tasks` skill)
4. **Execute** — Implement one approved task at a time (`spec-execute` skill)
...
|----------|---------|------------|
| `references/memory-bank.yaml` (this skill's directory) | Storage schema, feature naming rules, phase configuration, state detection rules | Stop and tell the user the skill install is incomplete |
| `specs/` | Existing specs, used for state detection | Warn and continue — treat as a new project with no specs yet |
| `specs/{feature}/requirements.md` | What to build (required for design, tasks, execution) | Warn and continue — start at the requirements phase instead |
| `specs/{feature}/design.md` | How to build it (required for tasks, execution) | Warn and continue — start at the design phase instead |
| `specs/{feature}/tasks.md` | Current progress (required for execution) | Warn and continue — start at the tasks phase instead |

## Activation Triggers

Use this flow when the user's input EXPLICITLY:
...
When you need to ask the user a question (e.g., clarifying vague input), use a structured question tool if the host environment provides one. If none is available, ask directly in your response text.
...
| Skill | Purpose |
|-------|---------|
| `spec-requirements` | Generate/update requirements document with EARS-format acceptance criteria. Output: `specs/{feature}/requirements.md`. Approval prompt: "Do the requirements look good? If so, we can move on to the design." |
| `spec-design` | Generate/update technical design document with architecture and data models. Precondition: requirements approved. Output: `specs/{feature}/design.md`. Approval prompt: "Does the design look good? If so, we can move on to the implementation plan." |
| `spec-tasks` | Generate/update implementation task list with coding tasks. Precondition: design approved. Output: `specs/{feature}/tasks.md`. Approval prompt: "Do the tasks look good?" |
| `spec-execute` | Execute a single task from the approved tasks list. Precondition: all three spec files exist. Output: code changes + updated task checkbox |
...
User invokes this skill with no feature named.
Action:
...
User: "Continue working on [feature]", or invokes this skill with no arguments
