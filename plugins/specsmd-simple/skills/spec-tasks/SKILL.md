---
name: spec-tasks
description: Use when the user asks to write or update the implementation plan for a feature spec whose design is approved. Produces `specs/{feature}/tasks.md` as a numbered coding-task checklist with requirement traceability and checkpoints.
license: MIT
metadata:
  version: "1.0.0"
  flow: simple
...
|----------|---------|------------|
| `specs/{feature}/requirements.md` | Requirement IDs for traceability references | Stop and tell the user requirements must exist and be approved first |
| `specs/{feature}/design.md` | The design the tasks implement | Stop and tell the user the design must exist and be approved first |
| `references/tasks-template.md` (this skill's directory) | Document structure for the tasks file | Warn and continue using the structure described below |
| `specs/{feature}/tasks.md` | Existing tasks, when updating rather than creating | Warn and continue — treat as a new task list |
...
- Design changes → invoke the `spec-design` skill
- Requirement changes → invoke the `spec-requirements` skill

**Forward**: On approval:

- Workflow complete
- User can now request task execution via the `spec-execute` skill

## Template Reference

See `references/tasks-template.md` in this skill's directory for full template structure.
