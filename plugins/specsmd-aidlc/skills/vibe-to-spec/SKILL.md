---
name: vibe-to-spec
description: Use when the user has a vibe-coded prototype — screenshots, mockups, HTML or design exports — and wants formal specs from it. Extracts a design system, component catalog, user flows, and derived requirements that feed AI-DLC Inception.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
- **Required**: The memory-bank layout - artifacts are written under `memory-bank/prototypes/` (full schema: `references/memory-bank.yaml` in the `inception` skill)
...
1. The `intent-create` skill can reference `derived-requirements.md`
2. The `requirements` skill can import and refine FR-P* items
3. The `context` skill can use the component catalog for system boundaries
4. The `units` skill can organize by identified flows
5. The `story-create` skill can generate stories from the user flow mapping

**REQUIRED NEXT SKILL**: after Checkpoint 3 is approved and the user chooses option 1, invoke the `intent-create` skill immediately (do not stop). Checkpoint 3 must be passed first.

---

## Construction Phase Reference

When a Construction bolt references this prototype:
...
5. **Use a frontend design skill if one is available** - when implementing UI components, invoke the `frontend-design` skill for production-grade frontend code that avoids generic AI aesthetics
