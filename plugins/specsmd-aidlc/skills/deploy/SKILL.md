---
name: deploy
description: Use when the user asks to deploy or promote a unit to dev, staging, or production during the AI-DLC Operations phase, or to roll back to a previous version. Deploys a built version with pre-checks, health waiting, and a recorded rollback path, gated by approval checkpoints.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
There is no separate rollback skill. To roll back, invoke this skill with the previous known-good version:

```text
deploy --unit="{unit}" --env="{env}" --version="{prev-version}"
```

Record the rollback in `deployment/history.md` the same way as any other deployment, noting the reason.
...
- → invoke the `verify` skill - verify deployment health
- → invoke the `monitor` skill - set up monitoring (after production)
