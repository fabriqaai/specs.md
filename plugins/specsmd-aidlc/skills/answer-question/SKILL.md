...
description: Use when the user asks a factual question about their own AI-DLC project — a bolt's status, what requirements or stories exist, the tech stack, architecture decisions, or deployment history. Answers from memory-bank artifacts with source citations.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
- **Required**: The memory-bank schema (`references/memory-bank.yaml` in the `inception` skill) - artifact paths
...
2. **Locate artifacts** using the memory-bank schema paths (`memory-bank/intents/`, `memory-bank/bolts/`, `memory-bank/standards/`, `memory-bank/operations/`)
...
- → invoke the `specsmd-status` skill - if the user wants to take action or see overall status
