...
description: Use when a bolt implements UI that must match an existing prototype, or the user asks to apply a prototype's design system and components to production code during the AI-DLC Construction phase. Produces implementation code plus a visual verification checklist.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
- The design system extracted from a prototype should be applied to code
...
**IMPORTANT**: If a `frontend-design` skill is available in the session, invoke it for high-quality, production-grade frontend code that avoids generic AI aesthetics.
