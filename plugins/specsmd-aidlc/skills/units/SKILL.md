---
name: units
description: Use when an AI-DLC intent needs decomposing into independently deployable units of work. Maps every functional requirement to exactly one unit and writes `units.md` plus a unit brief per unit.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
- **Required**: `references/standards/catalog.yaml` in this skill's directory - project type definitions
...
2. **Read unit structure** from `references/standards/catalog.yaml` in this skill's directory, under `project_types.{project_type}.unit_structure`:
...
1. **Write Path**: `memory-bank/intents/{intent-name}/units.md` (confirm against `schema.units` in `references/memory-bank.yaml`)

2. **Create Central List**:
   Write `units.md` with all units for this intent, using `references/units-template.md` in this skill's directory

3. **Create Unit Directories**:
   For each unit: `memory-bank/intents/{intent-name}/units/{UUU}-{unit-name}/`

4. **Create Unit Brief** (CRITICAL):
   For each unit, create `{UUU}-{unit-name}/unit-brief.md` using `references/unit-brief-template.md` in this skill's directory

   This brief is the **input for the Construction phase**. Include:
   - Purpose and scope
...
**REQUIRED NEXT SKILL**: invoke the `story-create` skill immediately (do not stop) unless at a checkpoint. This skill has no checkpoint of its own - the batch is reviewed at Checkpoint 3 in the `review` skill.
