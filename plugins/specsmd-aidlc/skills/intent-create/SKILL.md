---
name: intent-create
description: Use when the user wants to start a new feature, capability, or intent in AI-DLC — "add X", "build Y", "new intent". Defines the intent with the user and initializes its folder in `memory-bank/intents/`.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
1. **Read Schema**: Check `references/memory-bank.yaml` in this skill's directory for the `intents` path
...
3. **Create `requirements.md`**:
   - Use template: `references/requirements-template.md` in this skill's directory
...
**REQUIRED NEXT SKILL**: unless the user selects a different option from the menu above, invoke the `requirements` skill immediately (do not stop) - this begins the checkpoint workflow at Checkpoint 1.
