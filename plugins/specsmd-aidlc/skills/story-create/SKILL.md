---
name: story-create
description: Use when an AI-DLC unit needs user stories with acceptance criteria written from its assigned requirements. Creates one file per story under the unit's `stories/` folder and updates the story index.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
1. **Write Path**: `memory-bank/intents/{intent-name}/units/{unit-name}/stories/` (confirm against `schema.stories` in `references/memory-bank.yaml`)
...
3. **Create Story Files** (IMPORTANT - ONE FILE PER STORY):
   ...
   **Read the naming convention from `references/memory-bank.yaml` in this skill's directory**
   ...
   **DO NOT** create a single `stories.md` file with all stories
   ...
   Use template: `references/story-template.md` in this skill's directory

   *(`references/stories-template.md` shows the consolidated multi-story layout. It is retained for reading legacy artifacts only - do not generate it.)*
...
1. **Read Configuration**: Check `story-index` settings in `references/memory-bank.yaml` in this skill's directory
...
**REQUIRED NEXT SKILL**: invoke the `bolt-plan` skill immediately (do not stop) unless at a checkpoint. This skill has no checkpoint of its own - the batch is reviewed at Checkpoint 3 in the `review` skill.
