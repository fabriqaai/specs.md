---
name: requirements
description: Use when an AI-DLC intent needs its functional and non-functional requirements elicited and written to `requirements.md`. Runs the structured inquiry with two human checkpoints before artifact generation begins.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
Read these before starting. All paths are in the user's project.

| Artifact | Purpose | If missing |
|----------|---------|------------|
| `memory-bank/project.yaml` | Project type, for realistic requirement framing | Continue - the project is not initialized yet; `aidlc-init` creates it and defaults apply |
| `memory-bank/standards/tech-stack.md` | Technology context | Continue - optional |
| `memory-bank/intents/{intent}/requirements.md` | Existing draft to update | Continue - a new draft is written |

Project standards are loaded automatically by the skills that need them. Do not duplicate them in requirements.
...
1. **Write Path**: `memory-bank/intents/{intent-name}/requirements.md`

2. **Use Template**: `references/requirements-template.md` in this skill's directory
...
Required standards are loaded from `memory-bank/standards/` during Construction.
...
3. Proceed to generate the remaining artifacts (Context + Units + Stories + Bolts)

---

## Transition

After requirements are approved at Checkpoint 2, the remaining artifacts are generated as a batch:
...
**REQUIRED NEXT SKILL**: invoke the `context` skill immediately (do not stop) unless at a checkpoint. Checkpoint 2 must be passed first - never continue past a checkpoint without the user's answer.
