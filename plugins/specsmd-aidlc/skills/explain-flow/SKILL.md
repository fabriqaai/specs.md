---
name: explain-flow
description: Use when the user asks what AI-DLC is, how the flow works, what a bolt, unit, intent, or checkpoint means, how it differs from other methodologies, or where they are in the lifecycle. Explains AI-DLC and the user's current position.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
- **Optional**: Current project state (invoke the `specsmd-status` skill to determine it)
...
- **Bolt**: Time-boxed execution session (hours or days, not weeks)
...
- **Rapid Iteration**: Bolts are hours or days, not weeks
```
...
- → invoke the `specsmd-status` skill - if the user wants to proceed and needs the right next step
- → invoke the `answer-question` skill - if the user has more questions about their own project
