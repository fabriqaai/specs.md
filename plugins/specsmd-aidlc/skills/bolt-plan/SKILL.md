---
name: bolt-plan
description: Use when an AI-DLC unit's stories need grouping into bolts before Construction. Assesses complexity and dependencies, then creates a directory with a `bolt.md` per bolt under `memory-bank/bolts/`.
license: MIT
metadata:
  version: "1.0.0"
  flow: aidlc
...
3. **Bolt type definitions** (stages, artifacts, and stage prompts) ship with the `bolt-start` skill, which reads them when the bolt is executed. Planning only needs to record the correct `type` value in the bolt frontmatter.
...
1. **Write Path**: `memory-bank/bolts/{bolt-id}/` (confirm against `schema.bolts` in `references/memory-bank.yaml`)
...
3. **Create Directory + File Per Bolt**:
   ...
   **Naming Convention** (from `references/memory-bank.yaml`):
...
**REQUIRED NEXT SKILL**: invoke the `review` skill immediately (do not stop) unless at a checkpoint. The `review` skill presents the whole batch at Checkpoint 3.
