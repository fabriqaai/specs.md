...
1. **Intent → work item → bolt.** A bolt belongs to one intent and lives under that intent's folder. Draft bolts are optional. `bolt-execution` is the only bolt skill.
