---
name: spark
description: Use when the user asks to generate ideas on a topic, wants another batch, wilder options, or variations on an idea they liked, or invokes the spark command. Produces rapid batches of cross-domain ideas and compiles them into a Spark Bank.
license: MIT
metadata:
  version: "1.0.0"
  flow: ideation
...
<triggers>
  ...
  - The `ideation` skill routes to Spark
  - User invokes the `spark` command
</triggers>
...
<protocols>
  Read these shared protocols in this skill's directory before generating:
  - `references/protocols/anti-bias.md` — Domain wheel, diversity enforcement
  - `references/protocols/deep-thinking.md` — Multi-step reasoning per batch
  - `references/protocols/interaction-adaptation.md` — Generate/elicit/co-build rules
  - `references/protocols/diverge-converge.md` — Osborn-Parnes dual thinking
</protocols>
...
<flow>
  ...
  <step n="2" title="Deep Thinking">
    <action>Execute the deep thinking protocol (see `references/protocols/deep-thinking.md`)</action>
  ...
  <step n="6" title="Generate Spark Bank">
    <action>Compile all ideas into a Spark Bank document using the template `references/templates/spark-bank.md.hbs` in this skill's directory</action>
...
<output_artifacts>
  ...
  |----------|----------|----------|
  | Spark Bank | `.specs-ideation/sessions/{id}/spark-bank.md` | `references/templates/spark-bank.md.hbs` |
</output_artifacts>
