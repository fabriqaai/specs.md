---
name: flame
description: Use when the user asks to evaluate, score, compare, stress-test, or shortlist ideas they already have or that Spark produced, or invokes the flame command. Produces a multi-perspective evaluation, impact and feasibility scores, and a ranked shortlist.
license: MIT
metadata:
  version: "1.0.0"
  flow: ideation
...
<triggers>
  ...
  - The `ideation` skill routes to Flame
  - User invokes the `flame` command
</triggers>
...
<protocols>
  Read these shared protocols in this skill's directory:
  - `references/protocols/interaction-adaptation.md` — Generate:60% / Elicit:15% / Co-build:25%
  - `references/protocols/diverge-converge.md` — Convergent mode with structured divergent extensions
</protocols>
...
<flow>
  ...
  <step n="5" title="Shortlist and Flame Report">
    ...
    <action>Generate Flame Report using the template `references/templates/flame-report.md.hbs` in this skill's directory</action>
...
<output_artifacts>
  ...
  |----------|----------|----------|
  | Flame Report | `.specs-ideation/sessions/{id}/flame-report.md` | `references/templates/flame-report.md.hbs` |
</output_artifacts>
