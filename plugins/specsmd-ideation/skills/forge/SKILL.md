---
name: forge
description: Use when the user asks to shape, develop, flesh out, or pitch a chosen idea into a full concept, or invokes the forge command. Produces a polished concept brief covering the vision, an implementation path, and risks with mitigations.
license: MIT
metadata:
  version: "1.0.0"
  flow: ideation
...
<triggers>
  ...
  - The `ideation` skill routes to Forge
  - User invokes the `forge` command
</triggers>
...
<protocols>
  ...
  - `references/protocols/interaction-adaptation.md` — Generate:40% / Elicit:20% / Co-build:40%
  - `references/protocols/diverge-converge.md` — Alternating diverge-converge (Disney pattern)
</protocols>
...
<flow>
  ...
  <step n="5" title="Generate Concept Brief">
    ...
    <action>Generate using the template `references/templates/concept-brief.md.hbs` in this skill's directory</action>
...
<output_artifacts>
  ...
  |----------|----------|----------|
  | Concept Brief | `.specs-ideation/sessions/{id}/concept-briefs/{name}.md` | `references/templates/concept-brief.md.hbs` |
</output_artifacts>
