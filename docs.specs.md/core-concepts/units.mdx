...
```mermaid
flowchart TB
    REG[Registration] -->|creates| SESSION[Session Mgmt]
    LOGIN[Login] --> SESSION
    PWD[Password Mgmt] -->|uses| SESSION

    style REG fill:#818CF8,stroke:#6366F1,color:#fff
    style LOGIN fill:#34D399,stroke:#10B981,color:#fff
    style SESSION fill:#8B5CF6,stroke:#7C3AED,color:#fff
    style PWD fill:#FBBF24,stroke:#F59E0B,color:#fff
```
...
## Unit vs Microservice

| Aspect | Unit | Microservice |
|--------|------|--------------|
| **Granularity** | Logical grouping | Deployment unit |
| **Coupling** | Loose at design | Independent runtime |
| **Communication** | In-process or API | Always network |
| **Ownership** | Same team | Could be different teams |

Units can become microservices, but not all units should be. Use units for logical organization; use microservices for operational independence.
