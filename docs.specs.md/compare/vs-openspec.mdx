...
| **Best For** | Greenfield + brownfield | Brownfield-first |
...
| **Token Efficiency** | Memory Bank context | Spec deltas only (very efficient) |
| **Design Integration** | DDD as core | None |
| **Rituals** | Mob Elaboration, Mob Construction | None |
| **Learning Curve** | Moderate | Low |
| **Installation** | CLI + configuration | Simple `npm install` |
...
**specs.md**: Memory Bank provides structured context, but sends more comprehensive context to ensure AI understands the full picture.
...
**specs.md**: Supported via "model elevation" - AI first converts existing code to semantic models (domain components, relationships) before making changes.
...
| Architecture changes | Limited | Excellent |
...
<Steps>
  <Step title="Use specs.md for Planning">
    Run Inception phase to decompose complex features into Units
  </Step>
  <Step title="Use OpenSpec for Execution">
    For each Unit, use OpenSpec's change-centric approach for implementation
  </Step>
  <Step title="Return to AI-DLC for Operations">
    Use Operations phase for deployment and monitoring
  </Step>
</Steps>
