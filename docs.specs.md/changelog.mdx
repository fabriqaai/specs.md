...
<Update label="February 22, 2026" description="v0.1.49" tags={["FIRE", "CLI", "Release"]}>
### Added

- Added manual `workflow_dispatch` support to the npm package release workflow for fast recovery when a publish run is interrupted
...
- NPM release workflow now avoids non-fast-forward publish failures by using release concurrency plus rebase-and-retry push logic for version bump commits
- FIRE documentation now consistently uses `run-<worktree>-NNN` examples to avoid run ID conflicts across parallel worktree-based agent sessions
</Update>
