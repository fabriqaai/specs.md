...
flowchart TB
    P{{Prerequisites}}:::build -->|Construction complete?| A(Build Artifacts):::build
    A --> G1{{"✋ Gate 1"}}:::check
    G1 -->|User approves| B(Deploy to Dev):::plan
    B --> C(Deploy to Staging):::plan
    C --> G2{{"✋ Gate 2"}}:::check
    G2 -->|User approves| D(Verify Staging):::plan
    D --> E(Deploy to Production):::warn
    E --> G3{{"✋ Gate 3"}}:::check
    G3 -->|User approves| F(Verify Production):::warn
    F --> G(Setup Monitoring):::build
    G --> G4{{"✋ Gate 4"}}:::check
    G4 -->|User approves| H((Complete)):::done
