...
```mermaid
flowchart TB
    Start[/specsmd-master-agent/] --> Check{Standards exist?}

    Check -->|No| New[New User Flow]
    Check -->|Yes| Return[Returning User Flow]

    subgraph New[New User]
        E[Explain AI-DLC] --> Init[Project Init]
    end

    subgraph Return[Returning User]
        Analyze[Analyze Context] --> Route[Route to Agent]
    end

    Route --> IA[Inception Agent]
    Route --> CA[Construction Agent]
    Route --> OA[Operations Agent]

    style Check fill:#F472B6,stroke:#EC4899,color:#fff
