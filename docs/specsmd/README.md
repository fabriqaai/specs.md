...
- The flow's tooling (the `specsmd` plugin's `flow-runtime` scripts) is the only state writer. Install locally with `/plugin marketplace add <path-to-this-repo>` then `/plugin install specsmd@specsmd`.
