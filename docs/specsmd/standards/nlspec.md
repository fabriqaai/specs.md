...
If a caller can tell two implementations apart, the spec is not done. Hunt **return**, **surfaces**, **set rule**, **shape**, and **credential**. Silence is not a freedom. Two stories in one tree are a contradiction. Product code does not start while a hunt is open. Details live with the flow-runtime `caller-contracts.md` reference.
